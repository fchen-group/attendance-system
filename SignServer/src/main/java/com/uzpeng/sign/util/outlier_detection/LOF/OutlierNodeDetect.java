...
public class OutlierNodeDetect {
     ...
     * 计算每个点的可达距离  ReachDis
     * @param kdAndKnList
     * 计算每个遍历节点p中 邻域的五个节点o的 可达距离 比较对象是 o跟p的真实距离Distance 以及 o自己的k-Distance
     * ！！！其实是设置 每个遍历节点中 领域的 五个节点o的可达距离
     */
    private void calReachDis(List<DataNode> kdAndKnList) {
        for (DataNode node : kdAndKnList) {                   //node     即 p
            List<DataNode> tempNodes = node.getkNeighbor();
            for (DataNode tempNode : tempNodes) {             //tempNode 即 o
                //获取tempNode点的k-距离                       //点o到点p的第k可达距离
                double kDis = getKDis(tempNode.getNodeName(), kdAndKnList);
                //reachdis(p,o)=max{ k-distance(o),d(p,o)}
                if (kDis < tempNode.getDistance()) {          //tempNode.getDistance()存的是 点o到p的实际距离
